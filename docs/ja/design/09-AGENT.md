...
4. Act:     バッチ内のアクションを順に実行（各アクションが1 tickを消費）
            予期しないイベント（フォールト、割り込み）で残りのバッチを破棄し、次のthinkへ
...
初期エージェント数**N**は固定ではない。NEXUSが予算とレート制限から自律的に決定する（最小4 —— [13-SUMMONER.md](./13-SUMMONER.md) §4.3の`plan_civilization`参照）。役割は同§4.4の固定比率で分配され、各エージェントの特性は役割アーキタイプの周辺から多様性を持ってサンプリングされる（同一役割のエージェントが同じ特性を持たないこと）。全員が初期レピュテーション0.5、初期助成金100 ⚡で開始する。
...
- 既存エージェントがスポンサー（50 ⚡ + SPAWN_AGENTガバナンス投票 —— Epoch 2以降で解放、[00-MASTER.md](./00-MASTER.md) §5参照）
- 人口が4を下回る（緊急生成、投票不要の「物理法則」）
- エポック進行（最大容量が初期数Nからエポックごとに4増加）
