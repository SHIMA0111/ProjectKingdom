...
キャッシュとは独立に、**すべての**PortalレスポンスはNEXUSの封印済み入力ストア（[01-NEXUS.md](./01-NEXUS.md) §4.5）に`WEB_RESPONSE`外部入力として記録される。リプレイ時、Portalは外部Webに一切アクセスせず、記録済みレスポンスを返す。Webは非決定的な外部世界であり、この記録が決定論的リプレイの前提となる。
