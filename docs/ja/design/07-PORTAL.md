...
共有キャッシュヒットは通貨コストが無料（要求エージェントに対して0 ⚡、1 tick —— §7のコスト表参照）。

### 3.4 決定論的リプレイのための記録

**すべての外部フェッチのレスポンス**は、NEXUSの封印済み入力ストア（[01-NEXUS.md](./01-NEXUS.md) §4.5）に`WEB_RESPONSE`外部入力として記録される。キャッシュヒットは記録済みレスポンスの再利用であり、新たな外部入力を生まない（したがって再記録もされない）。リプレイ時、Portalは外部Webに一切アクセスせず、記録済みレスポンスを返す。Webは非決定的な外部世界であり、この記録が決定論的リプレイの前提となる。
